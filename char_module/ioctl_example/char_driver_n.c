...
// Function which performs the IOCTL requests from the user. (Dummy for now -_-)
static long dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	pr_info("IOCTL fucntion has been invoked.\n");
	switch (cmd) {
		case RD_DATA: {
			copy_to_user((int32_t*)arg, &val, sizeof(val));
			pr_info("Data copied to the user space.\n");
			break;
		}
		case WR_DATA: {
			copy_from_user(&val, (int32_t*)arg, sizeof(val));
			pr_info("Data copied into the driver.\n");
			break;
		}
		default: {
			return -ENOTTY;
